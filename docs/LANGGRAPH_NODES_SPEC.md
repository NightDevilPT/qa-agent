...
* **State Update Payload Example (Python FastAPI Project):**
  ```json
  {
    ...
    "logic_signatures": {
      "control_flow_keywords": ["if", "elif", "else", "for", "while", "try", "except", "return", "raise", "with"],
      "async_and_event_signatures": ["async def", "await", "httpx", "aiohttp", "asyncio"],
      "framework_handler_signatures": ["@app.get", "@app.post", "@router.get", "APIRouter()", "BaseModel", "Depends"],
      "database_query_signatures": ["SessionLocal", "db.query", "select(", "filter("],
      "negative_non_testable_signatures": ["__all__ = ", "settings = ", "pass"]
    },
    "language_rules": {
      "non_testable_extensions": [".pyc", ".pyo", ".pyd", ".png", ".env", "__init__.py"]
    }
  }
  ```
...
### Node 4: `classify_files_node` (Transitive Content Hash & AST Structural Clustering)
* **LLM Usage:** ⚡ **NO LLM (0 Tokens)**
* **State Read:** `workspace_dir`, `language_rules`, `logic_signatures`, `completed_files`
* **State Write:** `testable_files`, `non_testable_files`, `ast_clusters`
* **Responsibility:**
  1. **Transitive Content Hash Check:** Computes `transitive_hash = sha256(content + sha256(imported_dependencies))`. If `mathUtils.py` changes, its hash change automatically invalidates `paymentService.py` and `checkoutController.py`!
  2. If `transitive_hash` matches a previously passed test in `.temp/state.json`, skips file (`reason: "UNCHANGED_TRANSITIVE_HASH_SKIPPED"`).
  3. **AST Structural Clustering:** Groups testable files into structural shape clusters (e.g., 40 CRUD controllers) to enable template-based test adaptation in Node 9.
* **State Update Payload Example:**
  ```json
  {
    "testable_files": [
    ...
    "non_testable_files": [
      {
        "source_file": "app/utils/math.py",
        "reason": "Transitive content sha256 hash unchanged from previous run (File + dependencies unchanged)",
        "skipped_by": "UNCHANGED_TRANSITIVE_HASH_SKIPPED"
      }
    ]
  }
  ```
